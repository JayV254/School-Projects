...
void CodeGen::visitGlobal(Global *global)
{
    ...
    type_t type = currtype;
    visitIdent(global->ident_);
    Ident var_name = currid;

    symbols.insert(Symbol(var_name, type, code.pos()));
...
void CodeGen::visitSIfThen(SIfThen *sifthen)
{
    ...
    sifthen->stm_->accept(this);  // If body
    code.at(patchloc) = code.pos() - (patchloc - 1);
}

void CodeGen::visitSIfe(SIfe *sife)
{
    sife->exp_->accept(this);
    code.add(I_JR_IF_FALSE);  // Jump past the body.
    code.add(0);
    int patchloc = code.pos() - 1;

    sife->stm_1->accept(this);   //body
    code.at(patchloc) = code.pos() - (patchloc -1);
    ...
    sife->stm_2->accept(this);
    code.at(patchloc) = code.pos() - (patchloc - 1);
}

void CodeGen::visitSIfThenE(SIfThenE *sifthene)
{
    sifthene->exp_->accept(this);
    code.add(I_JR_IF_FALSE);  // Jump past the body.
    code.add(0);
    int patchloc = code.pos() - 1;

    sifthene->stm_1->accept(this);   //body
    code.at(patchloc) = code.pos() - (patchloc -1);
...
{
    sfor->exp_1->accept(this); // sets the iterator = to whatever you want but the iterator must be previously declared
    int looploc = code.pos(); // Beginning of test- Marked location
    sfor->exp_2->accept(this); // generate code for second expression 
    code.add(I_JR_IF_FALSE);  // If first expression false jump past body
    code.add(0);
    int patchloc = code.pos() - 1;

    sfor->stm_->accept(this); // Body
    sfor->exp_3->accept(this); // second expression
    code.add(I_JR);
...
void CodeGen::visitCall(Call *call)
{
    ...
    string function_name = currid;
    if (!symbols.exists(currid))
        throw UnknownFunc(currid);

    // funargs is number of args expected for this function entry in the symbol table ----ADDED BY JESSE----
    int funargs = symbols[currid]->numargs();
    ...
    // if number of passed args is not the same as declaration, throw arg error ---JESSE---
    if(call->listexp_->size() != funargs) 
	throw ArgError(function_name);
    code.add(I_CALL);
    code.add(level);
    code.add(addr);
