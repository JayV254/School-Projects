...
// throw error function for incorrect number of arguments passed into function call ---ADDED BY LENNY---
class  ArgError : public std::logic_error
{
public:
    ArgError(const std::string &name)
	: logic_error("Number of passed arguments does not match function declaration for function \"" + name + "\"")
    {}
};
...
public:
    ...
    void visitSIf(SIf *p);
    void visitSIfe(SIfe *p);
    void visitSIfThen(SIfThen *p);
    void visitSIfThenE(SIfThenE *p);
