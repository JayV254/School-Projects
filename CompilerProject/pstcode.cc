...
void PstackCode::begin_prog(SymbolTable fvsyms)
{
...
void PstackCode::prolog(SymbolTable &fvsyms)
{
	...
	// getnum() function added (11/30) 
        fvsyms.insert(Symbol("getnum", TY_FUNC, pos(), 0));
        //after modifying Symbol table to handle num-of-args
        //as described in FAQ-2017.pdf: Function Argument (signature) checking:
        // What goes into checking function actual arguments
        //versus function definition?
        //the above line will change to 
        // fvsyms.insert(Symbol("getnum", TY_FUNC, pos(), 0));
        //to indicate the number of arguments -- the last 0)
        add(I_VARIABLE);
        add(0);
        add(-1);
        add(I_READ);
	...
	fvsyms.insert(Symbol("exit", TY_FUNC, pos(), 0));
	add(I_ENDPROG);
